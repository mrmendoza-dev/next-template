...
export const HomeView = () => {
	...
	const health = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(resolvePublicUrl(`${API_V1_PREFIX}/health`));
			if (!res.ok) {
	...
	const examples = useQuery({
		queryKey: ["api", "v1", "examples"],
		queryFn: async () => {
			const res = await fetch(resolvePublicUrl(`${API_V1_PREFIX}/examples`));
			if (!res.ok) {
			...
			if (!Array.isArray(data)) {
				throw new Error(
					"Examples API returned a non-array response. For split hosting, set NEXT_PUBLIC_API_URL to your API origin, or ensure GET /api/v1/examples returns a JSON array.",
				);
			}
		...
		<div className="space-y-8 p-4">
			...
			>
				...
				<p className="text-sm text-muted-foreground">
					Live data from{" "}
					<code className="text-foreground">GET {API_V1_PREFIX}/health</code>{" "}
					and{" "}
					<code className="text-foreground">GET {API_V1_PREFIX}/examples</code>.
					Refreshes every 5s while healthy. Run{" "}
				...
				{showStaticDeployHint ? (
					...
					>
						...
						<code className="text-foreground">{API_V1_PREFIX}/*</code> (default
						for full-stack Next). Static exports or split frontends may need{" "}
					...
					<div className="rounded-lg border bg-background p-4">
						...
						<p className="text-sm font-mono" data-testid="api-health-status">
							...
							{health.isPending
								...
								: health.isError
									? showStaticDeployHint
										? `unreachable (check ${API_V1_PREFIX} or NEXT_PUBLIC_API_URL)`
										: "unreachable"
									: (health.data?.status ?? "—")}
						</p>
					</div>
